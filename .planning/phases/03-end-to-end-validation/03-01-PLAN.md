...
# Plan 03-01: Create `test_grid.py` End-to-End Validation Script

**Goal:** Tao file `test_grid.py` tai project root, theo pattern cua `test_api.py`, verify tat ca 5 SC cua Phase 3.

**File created:** `test_grid.py` (project root, cung cap voi `test_api.py`)

---

## Task A: File skeleton, imports, helpers, and SC-5 (mock call count)

<read_first>
- test_api.py (pattern to follow: env var, client init, print-based output)
- grid_translator.py (public API: translate_with_grid, private: _translate_single_tile, _split_tiles, _stitch_tiles; constants)
</read_first>

<action>
Create `test_grid.py` at project root with these contents:
...
- SC-5 and SC-4 run first (no API key needed, mock-based)
- SC-1, SC-2, SC-3 need GEMINI_API_KEY and optionally test_images/
- Graceful skip when test_images/ is empty (D-03)
- SC-5 uses unittest.mock.patch on `grid_translator._translate_single_tile` (D-05)
- SC-4 tests RGBA through pipeline AND simulates both web (JPEG save) and CLI (PNG save) paths
- Print-based output following test_api.py pattern
- Exit code 0 for all-pass/skip, 1 for any fail
</action>

<acceptance_criteria>
1. `test_grid.py` exists at project root: `test -f test_grid.py`
2. File imports unittest.mock: `grep "from unittest.mock import" test_grid.py` returns match
3. SC-5 mock function exists: `grep "def test_sc5_mock_call_count" test_grid.py` returns match
4. SC-5 patches correct target: `grep "patch('grid_translator._translate_single_tile'" test_grid.py` returns match
5. SC-5 checks call_count: `grep "mock_fn.call_count" test_grid.py` returns match
6. SC-5 tests all 3 grid sizes: `grep "test_cases = \[(1, 1), (2, 4), (3, 9)\]" test_grid.py` returns match
7. SC-4 function exists: `grep "def test_sc4_rgba_no_jpeg_error" test_grid.py` returns match
8. SC-4 creates RGBA image: `grep "mode='RGBA'" test_grid.py` returns match
9. SC-4 tests JPEG save: `grep "format='JPEG'" test_grid.py` returns match
10. SC-1 function exists: `grep "def test_sc1_grid_comparison" test_grid.py` returns match
11. SC-1 graceful skip: `grep "SKIP: thu muc test_images/" test_grid.py` returns match
12. SC-2 function exists: `grep "def test_sc2_batch_web" test_grid.py` returns match
13. SC-3 function exists: `grep "def test_sc3_cli_batch" test_grid.py` returns match
14. SC-3 calls translate_images: `grep "from image_translator import translate_images" test_grid.py` returns match
15. Main entry point: `grep 'if __name__ == "__main__"' test_grid.py` returns match
16. GEMINI_API_KEY check: `grep "GEMINI_API_KEY" test_grid.py` returns match
17. get_test_images helper with graceful skip: `grep "def get_test_images" test_grid.py` returns match
18. No pytest import: `grep "import pytest" test_grid.py` returns NO match
19. No new dependencies (only stdlib + PIL + google.genai): `grep "^from\|^import" test_grid.py` shows only os, io, sys, pathlib, unittest.mock, PIL, google.genai, grid_translator, image_translator
20. `python -c "import ast; ast.parse(open('test_grid.py').read()); print('SYNTAX OK')"` prints SYNTAX OK
</acceptance_criteria>
...
After task A is complete, run:

```bash
# 1. Syntax check
python -c "import ast; ast.parse(open('test_grid.py').read()); print('SYNTAX OK')"

# 2. SC-5 and SC-4 (no API key needed)
python -c "
from test_grid import test_sc5_mock_call_count, test_sc4_rgba_no_jpeg_error
r5 = test_sc5_mock_call_count()
r4 = test_sc4_rgba_no_jpeg_error()
print(f'SC-5: {r5}, SC-4: {r4}')
assert r5 == True, 'SC-5 failed'
assert r4 == True, 'SC-4 failed'
print('OFFLINE TESTS PASS')
"

# 3. Full run (needs GEMINI_API_KEY)
python test_grid.py
```
...
- [ ] `test_grid.py` exists at project root, parseable Python, no syntax errors
- [ ] SC-5 verified: `unittest.mock.patch('grid_translator._translate_single_tile')` used; grid_n=1 -> 1 call, grid_n=2 -> 4 calls, grid_n=3 -> 9 calls checked via `call_count`
- [ ] SC-4 verified: RGBA (mode='RGBA') image passed through `translate_with_grid()` for grid_n=1,2,3; JPEG save tested with RGBA->RGB conversion; no OSError
- [ ] SC-1 verified: `translate_with_grid()` called with grid_n=1, 2, 3 on real game UI image from `test_images/`; graceful skip if directory empty/missing
- [ ] SC-2 verified: `translate_with_grid()` called 3 times with grid_n=2; all 3 must succeed; output size == input size
- [ ] SC-3 verified: `translate_images()` called directly with grid_n=2 on 5 images; output files exist; size matches input +-1px
- [ ] Graceful skip (D-03): when `test_images/` is empty or missing, SC-1/SC-2/SC-3 print warning and skip (not crash)
- [ ] No new dependencies: only stdlib (os, io, sys, pathlib, unittest.mock, tempfile, shutil), PIL, google.genai
- [ ] Follows test_api.py pattern: GEMINI_API_KEY env var, genai.Client(api_key=...), print-based output
- [ ] Exit code: 0 when all pass/skip, 1 when any fail
