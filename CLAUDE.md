...
**BẮT BUỘC dùng MCP Auggie (`mcp__auggie__codebase-retrieval`) làm công cụ TÌM KIẾM ĐẦU TIÊN** cho MỌI tác vụ liên quan đến code. Không được dùng Grep/Glob/Read để tìm kiếm trước khi thử Auggie.

- **Luôn dùng Auggie khi:** tìm function/class, hiểu luồng xử lý, tìm file liên quan, hỏi về cấu trúc, tìm pattern, tìm nơi implement feature, tìm test liên quan
- **Chỉ dùng Grep khi:** cần tìm TẤT CẢ occurrences của một identifier đã biết chính xác (ví dụ: tìm mọi chỗ gọi `translate_with_grid`)
- **Chỉ dùng Read khi:** đã biết chính xác file path và cần đọc nội dung cụ thể
- **Chỉ dùng Glob khi:** tìm file theo pattern tên (ví dụ: `**/*.py`)
- **directory_path:** Luôn dùng `d:\_Dev\GoodFirstIsuuses\ImagiTranslate` làm directory_path cho Auggie
...
### Output Format
- **Web**: giữ nguyên format input (PNG → PNG, WebP → WebP, JPEG → JPEG); RGBA→RGB chỉ khi xuất JPEG
...
- **Output**: Web giữ nguyên format input (PNG/WebP/JPEG); CLI giữ nguyên extension gốc
...
- **Web**: Preserves input format (PNG → PNG, WebP → WebP, JPEG → JPEG); RGBA→RGB only for JPEG output
...
| **Output Format** | Preserves input format (PNG/WebP/JPEG) | Preserve extension |
